...
{
  uint64_t res;
  if ( (res = Wrap32::wrap( value, isn ).unwrap( isn, checkpoint )) != value ) {
    ostringstream ss;

    res = Wrap32::wrap( value, isn ).unwrap( isn, checkpoint );
    ...
    ss << "  res is " << res <<"\n";
    ss << "  (Difference between value and res is " << value - res << ".)\n";
    ss << "  (Difference between value and res is " << res - value << ".)\n";
