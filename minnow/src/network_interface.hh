...
public:
  ...
  static bool compare_ethernet_address(const EthernetAddress& a,const EthernetAddress& b) {
    for(int i=0;i<6;i++){
...
private:
  ...
  // Mapper from IP address to Ethernet address
  struct ArpEntry
  {
    EthernetAddress ethernet_address;
    size_t last_time_ms;
  };
  static constexpr size_t arp_max_time = 30000;
  static constexpr size_t req_max_time = 5000;
  std::unordered_map<uint32_t, ArpEntry> arp_table_{};
  std::unordered_map<uint32_t, size_t> req_table_{};
  std::list<std::pair<uint32_t,InternetDatagram>> dgram_buffered_{};
};
